...
async def on_message(message: discord.Message):
    ...
    if message.guild and message.channel.category_id == CATEGORIA_ID:
        # Comandos são tratados separadamente
...
async def handle_staff_reply(message: discord.Message):
    # Ignora mensagens sem conteúdo e sem anexos
    # (evita disparar por embeds automáticos de links de convite)
    ...
    try:
        ...
        # Em vez de mandar mensagem de erro, só adiciona reação de aviso
        # Isso evita spam de "Usuario nao encontrado" ao enviar links de convite
        try:
            await message.add_reaction("⚠️")
        except Exception:
            pass
